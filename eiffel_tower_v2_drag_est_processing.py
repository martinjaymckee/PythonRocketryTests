...
#    print('{}, {}, {}, {}, {}, {}, {}, {}'.format(len(ts), len(azs), len(Vzs), len(Vhs), len(rhos), len(off_axis), len(Ts), len(ms_tot)))
    ...
    for t, az, Vz, Vh, rho, alpha, T, m in zip(ts, azs, Vzs, Vhs, rhos, off_axis, Ts, ms_tot):
        a_eff = az#g0+az
        Vmag = math.sqrt(Vz**2 + Vh**2)
        cd = (2*(m*g0*math.cos(alpha) - m*az + T)) / (rho * Sref * (Vmag**2))
        cds.append(0.83*cd)
        as_eff.append(a_eff)
        Vs.append(Vmag)
    Vs = np.array(Vs)
    ...
    Vs = Vs[condition]
    cds = cds[condition]
    as_eff = as_eff[condition]
    Ts = Ts[condition]     
    return ts, Vs, cds, ms_tot, as_eff, Ts
...
class CdEstFileHandler:
    ...
    def load_dir(self, directory):
        ...
        for filename in os.listdir(directory):  
            if filename.endswith('.cd_est'):
                try:
                    path = os.path.join(directory, filename)
                    result = self.load(path)
                    cd_est_results.append(result)
                except Exception as e:
                    print('Error loading Cd Estimation Data -> {}'.format(e))
...
if __name__ == '__main__':
    ...
    estimation_output_fmt = '{rocket}-{name}-{manufacturer}-{model}-{run}.cd_est'
    ...
    thrust_curve_cfg = {
        'num_random_passes': 25,
        'impulse_sd_percent': 5,
        'burnrate_sd_percent': 2.5,
        'noise_sd_percent': 10,
    }

    data_range_cfg = { # TODO: THIS NEEDS TO BE REPLACED WITH AN ADAPTIVE FUNCTION THAT LOOKS AT THE VERTICAL VELOCITY, OFF-AXIS ANGLE, AND IGNITION/APOGEE TIME....
        'ascent_start_percent': 3,
        'ascent_end_percent': 85
    }

    flight_data_list = load_flight_data_list("eiffel_tower_first_two_data_list.dat")

    fig_cd, ax_cd = plt.subplots(1, layout='constrained')
    fig_cd.suptitle('$C_d$ vs Velocity')
    ax_cd.set_title('Reference area measured at second platform', fontdict={'size': 6})
    ax_cd.set_xlabel('Velocity ($ms^{-1}$)')
    ax_cd.set_ylabel('Coefficient of Drag')    
    ...
    for flight in flight_data_list:
        ...
        if br_flight_log is not None:
            ...
            t_end = t_ignition + (data_range_cfg['ascent_end_percent'] * (t_apogee - t_ignition) / 100.0)
            idx_end = np.searchsorted(ts, t_end, side='right')
            t_start = t_ignition + (data_range_cfg['ascent_start_percent'] * (t_apogee - t_ignition) / 100.0)
            ...
            # azs = -1.0*br_flight_log['az'].values[idx_start:idx_end] - 9.80665
            # axs = br_flight_log['ax'].values[idx_start:idx_end]
            # ays = br_flight_log['ay'].values[idx_start:idx_end]
            # azs_filt = signal.savgol_filter(np.sqrt(azs**2 + axs**2 + ays**2), window_length=25, polyorder=3, delta=0.002)
            ...
            for idx in range(thrust_curve_cfg['num_random_passes']):

                eng = ref_eng.Scaled(
                    impulse_multiplier = random.gauss(1, thrust_curve_cfg['impulse_sd_percent']/100.0), 
                    burn_rate_multiplier = random.gauss(1, thrust_curve_cfg['burnrate_sd_percent']/100.0), 
                    noise_sd=random.gauss(0, thrust_curve_cfg['noise_sd_percent']/100)
                )
                eng.start(t_ignition)
                ts, Vzs_ref, cds, ms_tot, as_eff, Ts = run_cd_est(ts, azs_filt, Vzs, Vhs, rhos, off_axis, flight['m0'], eng, flight['Sref']) 
                sns.scatterplot(x=Vzs_ref, y=cds, alpha=0.01, color=flight['color'], ax=ax_cd)
    ...
    ax_cd.legend()
    fig_cd.savefig(r'D:\Workspace\Work\Apogee\Articles\Eiffel Tower Altitude\Images\part_II_cd_est_I65_J180.png', dpi=300)
