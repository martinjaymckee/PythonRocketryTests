...
class Vector3D:
    def __init__(self, vec, label=None, units=''):
        self.__ecef = np.array(vec)  # TODO: IMPLEMENT PROPER CONVERSIONS HERE....
        self.__label = label
        self.__units = units
        
    def __str__(self):
        x, y, z = self.__ecef[0], self.__ecef[1], self.__ecef[2]
...
class GeographicPosition:
    ...
    @property
    def llh(self):
        return coordinates.ECEFToLLH(self.ecef, ellipsoid=self.__ellipsoid, iters=self.__iters)
...
def heightAboveGround(pos, terrain):
    # TODO: NEED TO IMPLEMENT THE TERRAIN
    h_terrain = 0
