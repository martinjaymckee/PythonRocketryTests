...
def run_simulations(cfg, plots, pos = None):
 ...
 #   viridisN = mpl.colormaps['viridis'].resampled(N)
    ...
    sim_stats = []
    run_data = []
    extract_values = ['t', 'dt', 'm:mass', 'T:magnitude(forces.T)', 'D:magnitude(forces.D)', 'a_z:accel.z', 'a:magnitude(accel)', 'v:magnitude(vel)', 'h:alt(pos)']
    ...
    for idx, (dt, m, eng, ref_area, cd) in enumerate(zip(dts, masses, engs, ref_areas, cds)):
        ...
        print(' -- in {:0.1f} s  -- Maximum Altitude = {:0.1f} m'.format(total_time, h_max))
